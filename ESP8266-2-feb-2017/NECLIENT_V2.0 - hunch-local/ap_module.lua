...
ap_module.power_parameters=nil
ap_module.raw_parameters=nil
ap_module.srv_settings_json=nil
ap_module.pwflow_parameters=nil
...
function ap_module.init()
	...
	ap_module.server:listen(80,'192.168.4.1',function(scon) 
		...
		elseif string.find(request,"expecting") then
			print("expecting")
			client:send("<h3>Sent !</h3>\r\n")
		elseif string.find(request,"stop") then
			print("stop")
			client:send("<h3>Sent !</h3>\r\n")
		elseif string.find(request,"flow") then
			print("flow")
			if ap_module.pwflow_parameters ~= nil then
				client:send(ap_module.pwflow_parameters.."\r\n")
			else
				client:send("<h3>ERROR: Flow Scale Table not found</h3>\r\n")
			end	
		...
		elseif string.find(request,"aandeyo") then
			if ap_module.power_parameters ~= nil then
				client:send(ap_module.power_parameters.."\r\n")
			else
				client:send("<h3>ERROR:Not found</h3>\r\n")
			end
		elseif string.find(request,"rawvalues") then
			print("rawvalues")
			if ap_module.raw_parameters ~= nil then
				client:send(ap_module.raw_parameters.."\r\n")
			else
				client:send("<h3>ERROR:Raw values not found</h3>\r\n")
			end
		elseif string.find(request,"s1") then
				...
				client:send("<h3>Sent !</h3>\r\n")	
		elseif string.find(request,"777") then
			uart.write(0,27)
			print(request)
			client:send("<h3>Sent !</h3>\r\n")
		elseif string.find(request,"888") then
			uart.write(0,27)
			print(request)
			client:send("<h3>Sent !</h3>\r\n")
		elseif string.find(request,"699") then
			uart.write(0,27)
			print(request)
			client:send("<h3>Sent !</h3>\r\n")	
