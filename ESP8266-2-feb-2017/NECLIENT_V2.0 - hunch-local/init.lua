...
uart.on("data", "\r",function(data)
     if data ~= nil then
     	...
     	elseif string.find(data,"flow>") then
			local i = string.find(data,"flow>")
			apClient.pwflow_parameters = string.sub(data,i)	
		...
		elseif string.find(data,"pharrow>") then
			local i = string.find(data,"pharrow>")
			apClient.power_parameters = string.sub(data,i)
		elseif string.find(data,"raw>") then
			local i = string.find(data,"raw>")
			apClient.raw_parameters = string.sub(data,i)	
...
function update()
	...
	conn:connect(80,'192.185.113.30')
	conn:on("connection",function()

			uart.write(0,27)
			print("msg: Sending Data..\r\n")
			conn:send("GET /api/send_data.php?name="..apClient.info[2].."&password="..apClient.info[3].."&meter_id="..apClient.info[5]
